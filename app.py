...
def _infer_provider_key(provider_display_name: str, variant_key: str) -> str:
    """
    Infer the provider key ('gemini' or 'openai') from the provider display name
    or variant key. Used when loading experiments from database.
    
    Args:
        provider_display_name: The stored provider name (e.g., "Google Gemini", "OpenAI")
        variant_key: The stored model variant (e.g., "gemini-2.5-pro", "gpt-4o")
    
    Returns:
        Provider key ('gemini', 'openai') or None if cannot infer
    """
    # Try reverse-lookup by provider display name
    for pk, pv in model_config.PROVIDERS.items():
        if pv == provider_display_name:
            return pk
...
def initialize_all_session_state():
    ...
    defaults = {
        ...
        'load_experiment_id': None,  # For loading experiments from history
...
def show_active_experiments():
    ...
    for exp in filtered_experiments:
        ...
        with st.expander(
            ...
            with col2:
                # Continue conversation button - routes to main page with full features
                if st.button(f"▶️ Continue", key=f"continue_{exp['id']}"):
                    # Set flag to trigger experiment loading in conversation_page
                    st.session_state['load_experiment_id'] = exp['id']
                    
                    # Route to main conversation page (not limited continue page)
                    st.session_state['page'] = 'main'
                    
                    # Start in paused state so user has full control
                    st.session_state['paused'] = True
                    
                    # Clear any old messages from UI
                    st.session_state.messages = []
...
def conversation_page():
    ...
    # --- EXPERIMENT LOADER BLOCK ---
    # This block loads a saved experiment from the history viewer.
    # It runs BEFORE the sidebar and BEFORE the main UI is drawn.
    # This allows resuming an experiment with ALL features (pause, override, etc.)
    if 'load_experiment_id' in st.session_state and st.session_state.load_experiment_id is not None:
        exp_id = st.session_state.load_experiment_id
        
        # Get experiment from database
        exp = database.get_experiment_by_id(exp_id)
        
        if not exp:
            st.error(f"❌ Experiment #{exp_id} not found. It may have been deleted.")
            st.session_state.load_experiment_id = None
            return
        
        # Get all messages for this experiment
        messages_from_db = database.get_experiment_messages(exp_id)
        
        # Infer provider keys from stored data
        ...
        # Get human-friendly model display names
        ...
        # Populate UI message history (shows all previous messages in chat)
        st.session_state.messages = []
        for msg in messages_from_db:
            ...
            # Map stored sender_role to display name
            display_name = role
            ...
            if is_error:
                add_message_to_ui("error", "Error", f"[{error_type}] {content}")
            elif role == 'researcher' or role == 'researcher_interjection':
                add_message_to_ui("researcher", "Researcher", content)
            else:
                # Use display_name as the chat role
                add_message_to_ui(display_name, display_name, content)
        
        # Restore all session state from database
        st.session_state['experiment_id'] = exp_id
        st.session_state['experiment_name'] = exp.get('name') or f"Experiment #{exp_id}"
        st.session_state['conversation_mode'] = 'Manual'  # Safe default
        st.session_state.provider_a = a_pk if a_pk else 'gemini'
        st.session_state.provider_b = b_pk if b_pk else 'openai'
        ...
        st.session_state.model_a_display = a_var_disp
        st.session_state.model_b_display = b_var_disp
        st.session_state.max_turns = exp['max_turns']
        
        # Calculate turn count (excludes errors and researcher messages)
        non_error_messages = [m for m in messages_from_db if not m.get('is_error', 0)]
        researcher_messages = [m for m in non_error_messages if m['sender_role'] in ('researcher', 'researcher_interjection')]
        st.session_state.turn_count = (len(non_error_messages) - len(researcher_messages)) // 2
        
        # Rebuild conversation histories for both models (CRITICAL for continue to work)
        st.session_state.model_a_history, st.session_state.model_b_history = \
            rebuild_conversation_histories(exp, non_error_messages)
        
        # Determine next speaker and mid-turn state
        if non_error_messages:
            ...
            if last_role == exp['model_a_name'] or f"{exp['model_a_name']} (Override)" in last_role:
                # Model A just spoke, Model B should respond next
                st.session_state.next_speaker = st.session_state.model_b_display
                ...
                # Model B just spoke, Model A should respond next (new turn)
                ...
                # Started with researcher message, Model A goes first
                st.session_state.next_speaker = st.session_state.model_a_display
                st.session_state.last_speaker = None
                st.session_state.mid_turn_pause = False
        else:
            # No messages yet (edge case)
            st.session_state.next_speaker = st.session_state.model_a_display
            st.session_state.last_speaker = None
            st.session_state.mid_turn_pause = False
        ...
        # Clear the load flag (prevents re-loading on every rerun)
        st.session_state.load_experiment_id = None
        
        # Mark conversation as active (shows main chat UI instead of sidebar)
        st.session_state.conversation_active = True
        
        # paused is already True (set by Continue button)
        # This ensures the pause menu shows immediately
    ...
    # Only show sidebar if no conversation is active
    if not st.session_state.get('conversation_active', False):
        with st.sidebar:
            ...
            if st.button("🚀 Start New Experiment"):
                ...
                st.session_state.last_speaker = None
                st.session_state.conversation_active = True  # Mark conversation as active
                st.success(f"✅ Started experiment #{exp_id}: {experiment_name}")
                st.rerun()
        
        # If no conversation is active, don't show main UI
...
def main_app():
    ...
    # Page navigation - synced with session state
    page_options = ["💬 New Conversation", "📚 View History"]
    current_page_index = 1 if st.session_state.get('page') == 'history' else 0
    
    page = st.sidebar.radio("📍 Navigation", page_options, index=current_page_index)
    
    if page == "💬 New Conversation":
        st.session_state.page = 'main'  # Keep state in sync
        conversation_page()
    elif page == "📚 View History":
        st.session_state.page = 'history'  # Keep state in sync
        history_viewer_page()
